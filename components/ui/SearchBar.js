...
export default function SearchBar({
  ...
  return (
    <div className={cn('relative flex items-center group', className)}>
      {/* Search icon */}
      <Search
        className="absolute left-3 w-4 h-4 text-slate-400 dark:text-slate-500 pointer-events-none group-focus-within:text-indigo-500 transition-colors duration-150"
        aria-hidden="true"
      />
      ...
      {value && (
        <button
          ...
          className="absolute right-2.5 p-0.5 rounded-md text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all duration-150"
        >
