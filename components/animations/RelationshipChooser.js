    ...
    <div className="flex flex-wrap items-center gap-1.5 mb-5 min-h-[24px]">
      {answers.map((a, i) => (
        <span key={i} className="flex items-center gap-1.5">
          <span className="text-xs px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-medium border border-slate-200 dark:border-slate-600">
            {a}
          </span>
          {i < answers.length - 1 && (
...
function QuestionCard({ question, onAnswer }) {
  ...
  return (
    ...
    >
      ...
      {question.hint && (
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 italic">{question.hint}</p>
      )}
          ...
          <motion.button
            ...
            className={`relative flex items-center gap-3 px-4 py-3.5 rounded-xl border text-left text-sm font-medium transition-all duration-150 ${
              hoveredIdx === idx
                ...
                : 'border-slate-200 dark:border-slate-600 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:border-indigo-300 dark:hover:border-indigo-600'
            }`}
          >
            <span className={`flex-shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold transition-colors ${
              hoveredIdx === idx
                ...
                : 'bg-white dark:bg-slate-600 text-slate-500 dark:text-slate-400'
            }`}>
              {ans.display}
            </span>
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1.5 italic">{r.tagline}</p>
        </div>
        <motion.div
      ...
      <div className="mb-5">
        ...
        <pre className={`text-xs font-mono p-4 rounded-xl border ${r.border} bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 leading-relaxed overflow-x-auto whitespace-pre`}>
          {r.code}
        </pre>
      </div>
      ...
      <button
        onClick={onReset}
        className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-sm font-medium text-slate-600 dark:text-slate-300 hover:border-indigo-300 dark:hover:border-indigo-600 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-150"
      >
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 sm:p-6 transition-colors duration-200">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="text-base font-bold text-slate-900 dark:text-slate-100">Relationship Decision Tree</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Answer 2-3 questions to find the right field type</p>
        </div>
      </div>
