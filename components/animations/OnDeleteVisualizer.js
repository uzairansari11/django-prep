...
const TAB_COLORS = {
  rose: {
    ...
    inactive: 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-rose-300 dark:hover:border-rose-700 hover:text-rose-600 dark:hover:text-rose-400',
  },
  amber: {
    active: 'bg-amber-500 text-white border-amber-500',
    inactive: 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-amber-300 dark:hover:border-amber-700 hover:text-amber-600 dark:hover:text-amber-400',
  },
  sky: {
    active: 'bg-sky-600 text-white border-sky-600',
    inactive: 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-sky-300 dark:hover:border-sky-700 hover:text-sky-600 dark:hover:text-sky-400',
  },
  yellow: {
    active: 'bg-yellow-500 text-white border-yellow-500',
    inactive: 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-yellow-300 dark:hover:border-yellow-700 hover:text-yellow-600 dark:hover:text-yellow-400',
  },
  violet: {
    active: 'bg-violet-600 text-white border-violet-600',
    inactive: 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-violet-300 dark:hover:border-violet-700 hover:text-violet-600 dark:hover:text-violet-400',
  },
  slate: {
    active: 'bg-slate-700 text-white border-slate-700 dark:bg-slate-600 dark:border-slate-600',
    inactive: 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500 hover:text-slate-800 dark:hover:text-slate-200',
  },
};
  ...
  return (
    <AnimatePresence>
      {!isDeleted && (
        ...
        >
          <td className="px-3 py-2.5 text-sm font-mono text-slate-500 dark:text-slate-400">{author.id}</td>
          <td className="px-3 py-2.5 text-sm text-slate-800 dark:text-slate-200 font-medium">{author.name}</td>
    ...
    <AnimatePresence>
      {!isDeleted && (
        <motion.tr
          ...
          className="border-b border-slate-100 dark:border-slate-700/50"
        >
          <td className="px-3 py-2.5 text-sm font-mono text-slate-500 dark:text-slate-400">{book.id}</td>
          <td className="px-3 py-2.5 text-sm text-slate-800 dark:text-slate-200">{book.title}</td>
          <TableCell
...
export default function OnDeleteVisualizer() {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 sm:p-6 transition-colors duration-200">
      {/* Header */}
      <div className="flex items-center justify-between gap-4 mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-slate-100">on_delete Visualizer</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">Click Delete on an author to see what happens</p>
          </div>
        </div>
        <button
          onClick={handleReset}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-medium text-slate-600 dark:text-slate-400 hover:border-indigo-300 dark:hover:border-indigo-600 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-150"
        >
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="mb-5 p-3.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-700">
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 font-mono">id</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 font-mono">name</th>
              ...
              <tbody>
                ...
                {allAuthorsDeleted && (
                  <tr>
                    <td colSpan={3} className="px-3 py-4 text-center text-xs text-slate-400 dark:text-slate-500 italic">
                      (empty table)
                    </td>
                  </tr>
                )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-700">
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 font-mono">id</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 font-mono">title</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 font-mono">author_id</th>
                </tr>
              </thead>
              <tbody>
      ...
      <AnimatePresence>
        {selectedOption === 'DO_NOTHING' && Object.keys(bookStates).length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 8 }}
            transition={{ duration: 0.3 }}
            className="mt-4 p-3.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700"
          >
            <div className="flex items-start gap-2.5">
              <svg className="w-4 h-4 text-slate-500 dark:text-slate-400 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                <strong className="text-slate-800 dark:text-slate-200">DO_NOTHING</strong>: The author row is gone but the books still hold the old <code className="font-mono bg-slate-200 dark:bg-slate-700 px-1 rounded text-slate-800 dark:text-slate-200">author_id</code>. Accessing <code className="font-mono bg-slate-200 dark:bg-slate-700 px-1 rounded text-slate-800 dark:text-slate-200">book.author</code> via the ORM will raise <code className="font-mono bg-slate-200 dark:bg-slate-700 px-1 rounded text-slate-800 dark:text-slate-200">Author.DoesNotExist</code>. Only use with <code className="font-mono bg-slate-200 dark:bg-slate-700 px-1 rounded text-slate-800 dark:text-slate-200">db_constraint=False</code> for intentionally denormalized data.
              </p>
            </div>
          </motion.div>
        )}
      ...
      {/* Legend */}
      <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 grid grid-cols-2 sm:grid-cols-3 gap-2">
        {[
