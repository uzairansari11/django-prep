...
export default function QueriesPage() {
  ...
  const progress = Math.round((completedCount / queryTopics.length) * 100);

  const counts = useMemo(() => ({
    all: queryTopics.length,
    beginner: queryTopics.filter((t) => t.difficulty === 'beginner').length,
    intermediate: queryTopics.filter((t) => t.difficulty === 'intermediate').length,
    advanced: queryTopics.filter((t) => t.difficulty === 'advanced').length,
  }), []);

  const filtered = useMemo(() => {
    let topics = queryTopics;
      ...
      <div className="bg-white dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700/60">
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-5">
            ...
            <span>/</span>
            <span className="text-slate-900 dark:text-white font-medium">QuerySet & ORM</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-slate-500 dark:text-slate-400 max-w-lg">
                  Master Django&apos;s query API — from lazy evaluation and filtering to complex aggregations and raw SQL.
                </p>
              </div>
            </div>
            ...
            <div className="shrink-0 flex flex-col items-end gap-2">
              <span className="text-sm font-bold text-sky-600 dark:text-sky-400">
                {completedCount}/{queryTopics.length} completed
              </span>
              <div className="w-32">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-5">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {filtered.length === queryTopics.length
              ? `${queryTopics.length} topics`
              : `${filtered.length} of ${queryTopics.length} topics`}
            {search && (
